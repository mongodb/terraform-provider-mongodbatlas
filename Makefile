...
clean-atlas-org: ## Run a test to clean all projects and pending resources in an Atlas org, supports export DRY_RUN=false (default=true), MONGODB_ATLAS_CLEAN_PROJECT_PREFIXES=prefix1,prefix2, MONGODB_ATLAS_CLEAN_USER_EMAIL_PREFIXES=prefix1,prefix2
	@$(eval export MONGODB_ATLAS_CLEAN_ORG?=true)
	@$(eval export DRY_RUN?=true)
	go test -count=1 'github.com/mongodb/terraform-provider-mongodbatlas/internal/testutil/clean' -timeout 3600s -parallel=250 -run 'TestCleanProjectAndClusters|TestCleanOrgUsers' -v -ldflags="$(LINKER_FLAGS)"
...
# note: if you also want to generate documentation for autogen resources, first you need to register resources in the provider using `enable-internal-autogen` and
...
.PHONY: enable-internal-autogen
enable-internal-autogen: ## Enable use of internal autogen resources in the provider. Internal resources have the _api suffix.
	@go run tools/enable-internal-autogen/main.go
