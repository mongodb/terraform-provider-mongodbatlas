...
# e.g. run: make generate-resource-code resource_name=search_deployment
# resource_name is optional, if not provided all configured resources code will be generated
.PHONY: generate-resource-code ## Generate the code of autogenerated resources
generate-resource-code: 
	@go run ./tools/codegen/main.go $(resource_name)

# note: that this command does not automatically register resources in the provider, which is needed for docs generation
.PHONY: generate-resources # Generates all code and documentation for resources defined in tools/codegen/config.yaml
generate-resources:
...
enable-autogen: ## Enable use of autogen resources in the provider
	make add-lines filename=./internal/provider/provider.go find="project.Resource," add="customdbroleapi.Resource,databaseuserapi.Resource,pushbasedlogexportapi.Resource,searchdeploymentapi.Resource,"
