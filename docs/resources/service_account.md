...
The following example creates a Service Account without an initial secret, then creates its first secret explicitly.
...
To let Atlas generate a bootstrap secret instead, omit `without_initial_secret` and set `secret_expires_after_hours`. The secret value is returned only in the create response.
