...
The following example creates a Service Account and lets Atlas generate its first secret.
...
### Further Examples
- [Bootstrap secret](https://github.com/mongodb/terraform-provider-mongodbatlas/tree/master/examples/mongodbatlas_service_account/bootstrap_secret)
- [Without initial secret](https://github.com/mongodb/terraform-provider-mongodbatlas/tree/master/examples/mongodbatlas_service_account/without_initial_secret)
...
- `without_initial_secret` (Boolean) When true, creates the Service Account without generating an initial secret. `secret_expires_after_hours` must not be set when this is true. Defaults to false.
