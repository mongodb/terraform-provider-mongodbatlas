...
The configuration of encryption at rest with customer key management in GCP uses role-based authentication through Cloud Provider Access, which is the recommended approach over static service account keys.
...
For a complete example including GCP KMS resource creation and IAM binding setup, see the [GCP encryption at rest example](https://github.com/mongodb/terraform-provider-mongodbatlas/tree/master/examples/mongodbatlas_encryption_at_rest/gcp/).
