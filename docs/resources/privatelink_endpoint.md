...
* `supported_remote_regions` - (Optional) List of additional AWS regions that can connect to the endpoint service. Only applicable for AWS provider. The `region_name` is supported by default and must not be included.
