...
- `delete_on_create_timeout` (Boolean) Indicates whether to delete the created resource if a timeout is reached when waiting for completion. Default is `true`. If you suspect a transient error, wait before retrying to allow resource deletion to finish.
