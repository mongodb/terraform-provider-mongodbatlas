...
* `skip_default_alerts_settings` - (Optional) Flag that indicates whether to prevent Atlas from automatically creating organization-level alerts not explicitly managed through Terraform. Defaults to `true`. *Note: Organizations created with older versions of this provider may have this attribute set to `false`. In this case, users would need to manually update this attribute to `true` to prevent Atlas from automatically creating alerts.*
