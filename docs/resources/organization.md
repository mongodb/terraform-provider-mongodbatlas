...
* `skip_default_alerts_settings` - (Optional) Disables automatic alert creation. When set to `true`, Atlas doesn't automatically create organization-level alerts. By default, new Atlas Organizations created with the provider have this attribute set to `true`, which prevents the creation of alerts not explicitly managed through Terraform.
