...
~> **NOTE:** After importing a `mongodbatlas_log_integration` resource that uses OpenTelemetry, the first `terraform plan` shows a diff on `otel_supplied_headers` because the Atlas API redacts these values in GET responses. The diff is expected and resolves after running `terraform apply` once.
