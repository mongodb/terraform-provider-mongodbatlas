...
-> **NOTE:** Set `adaptive_settings_overrides` with `jsonencode`. The supported keys are `OVERLOAD_PROTECTION` and `SEARCH_OVERLOAD_PROTECTION`. Removing a key from the configured object resets that override. Destroying this resource resets all overrides to Atlas-managed defaults and does not delete the cluster.
