...
-> **NOTE:** Set `adaptive_settings_overrides` with `jsonencode`. Use `LOAD_SHEDDING` for Load Shedding and `SEARCH_LOAD_SHEDDING` for Search Load Shedding. Both settings accept Boolean values. Removing a key from the configured object resets that override. Removing the attribute or configuring an empty object resets all overrides. Destroying this resource also resets all overrides to Atlas-managed defaults and does not delete the cluster. Manage Adaptive Settings for a cluster with only one `mongodbatlas_cluster_adaptive_settings` resource. If multiple resources target the same cluster, each resource can overwrite the others, causing Terraform to repeatedly report and reapply changes.
