...
* `metric_name` - (Optional) Human-readable label identifying the metric to check. Used for stream processor alert types (e.g. `OUTSIDE_STREAM_PROCESSOR_METRIC_THRESHOLD`).
* `mode` - (Optional) How MongoDB Cloud computes the metric value. Set to `AVERAGE` for stream processor alerts.
* `operator` - The operator to apply when checking the current metric value against the threshold value.
  Accepted values are:
    - `GREATER_THAN`
    - `LESS_THAN`
