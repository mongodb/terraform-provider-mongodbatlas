...
**Known issue:** For clusters with more than one `replication_specs` entry, setting `use_effective_fields = true` currently has no effect. The resulting behavior is the same as leaving the attribute unset or setting it to `false`. We recommend that you do not enable this attribute for multi-shard clusters. See [Multi-shard clusters and topology changes](#multi-shard-clusters-and-topology-changes).
