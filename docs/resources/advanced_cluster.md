...
**Important:** Changes to `use_effective_fields` must be applied separately from any other cluster configuration changes. If you attempt to modify `use_effective_fields` along with other attributes in the same apply operation, the provider will return an error. To change this attribute, create a separate Terraform plan that only modifies `use_effective_fields`. Note that toggling this attribute may result in increased plan verbosity with `(known after apply)` markers for various attributes.
...
**Migration recommendation:** Adopt `use_effective_fields = true` in v2.x to prepare for the v3.x transition and benefit from improved auto-scaling workflows immediately. When enabling or disabling this attribute, apply it in a separate Terraform operation without other cluster configuration changes to avoid validation errors.
