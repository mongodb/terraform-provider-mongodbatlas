...
**Important:** To avoid validation errors, the value of `use_effective_fields` cannot be changed at the same time as `replication_specs` changes. Toggling this attribute will result in increased plan verbosity with `(known after apply)` markers for various attributes, which can be safely ignored. If you have existing `lifecycle.ignore_changes` blocks for spec fields (`instance_size`, `disk_size_gb`, `disk_iops`), remove them in a separate apply after enabling this flag.
...
**Important:** If you're enabling this flag on an existing cluster that has `lifecycle.ignore_changes` blocks for spec fields, follow this two-step process: (1) Enable `use_effective_fields = true` and apply, (2) Remove the `lifecycle.ignore_changes` blocks and apply again. The blocks are no longer needed and may interfere with the new behavior.
...
**Migration recommendation:** Adopt `use_effective_fields = true` in v2.x to prepare for the v3.x transition and benefit from improved auto-scaling workflows immediately. When enabling or disabling this attribute, ensure it's not combined with `replication_specs` changes in the same apply.
