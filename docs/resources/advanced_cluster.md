...
**Important:** To avoid validation errors, changes to `use_effective_fields` must be applied separately from other cluster configuration changes. Toggling this attribute might result in increased plan verbosity with `(known after apply)` markers for various attributes.
