...
* `tls_cipher_config_mode` - (Optional) The TLS cipher suite configuration mode. The `DEFAULT` mode uses the default cipher suites. The `CUSTOM` mode allows you to specify custom cipher suites for both TLS 1.2 and TLS 1.3. Possible values are `CUSTOM` or `DEFAULT`. To unset, this should be set back to `DEFAULT`.
