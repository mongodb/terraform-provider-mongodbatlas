...
**Important:** To avoid validation errors, the value of `use_effective_fields` can only be changed separately from other cluster configuration changes. Toggling this attribute will result in increased plan verbosity with `(known after apply)` markers for various attributes, which can be safely ignored.
