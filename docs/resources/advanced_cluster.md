...
- Tail append is the only shard topology change that this documentation presents as safe today. Existing shards must remain in the same order, and the operation must not rely on `use_effective_fields` or list-index `lifecycle.ignore_changes` to preserve Atlas-managed size values.
