...
* `intelligent_workload_management_policy_overrides` - (Optional) Map of Intelligent Workload Management (IWM) policy overrides. Each key is an IWM policy name (for example, `LOAD_SHEDDING`) and each value is a JSON-encoded override. Use `jsonencode` to construct each value.
