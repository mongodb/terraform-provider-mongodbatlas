...
* `wave_assignment` - (Optional) Integer that identifies the maintenance wave explicitly assigned to this project. Only editable when the organization's wave assignment mode is `MANUAL`. Remove this attribute from your configuration and run `terraform apply` to clear the explicit assignment and return to automatic wave derivation. See [`mongodbatlas_org_maintenance_settings`](org_maintenance_settings.md) to configure the organization-level wave assignment mode.
