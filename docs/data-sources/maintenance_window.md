...
* `wave_assignment` - Maintenance wave explicitly assigned to this project. Always returned when a value has been set, regardless of the organization's [`wave_assignment_mode`](../data-sources/org_maintenance_settings.md#attributes-reference). When the mode is `ENV_TAG_MAPPING`, the stored value is preserved but not used for scheduling. Switching back to `MANUAL` restores this value as the effective wave. Returns `null` when no explicit wave has been assigned.
