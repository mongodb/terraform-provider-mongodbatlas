...
* `wave_assignment` - Maintenance wave explicitly assigned to this project. Always returned when a value has been set, regardless of the organization's [`wave_assignment_mode`](../data-sources/org_maintenance_settings.md#attributes-reference). When the mode is `ENV_TAG_MAPPING`, the system preserves the stored value but does not use it for scheduling. Switching back to `MANUAL` restores this value as the effective wave. Returns `0` when no explicit wave has been assigned.
