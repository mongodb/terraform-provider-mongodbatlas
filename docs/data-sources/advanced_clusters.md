...
* `use_effective_fields` - (Optional) Controls how hardware specification fields are returned in the response. When set to true, the effective specs (`effective_electable_specs`, `effective_analytics_specs`, `effective_read_only_specs`) fields return the hardware specifications that the client provided. When set to false (default), the effective specs fields show the **current** hardware specifications. Cluster auto-scaling is the primary cause for differences between initial and current hardware specifications. **Note:** Effective specs are always returned regardless of the flag value and always report the **current** hardware specifications. See the resource documentation for [Auto-Scaling with Effective Fields](../resources/advanced_cluster.md#auto-scaling-with-effective-fields) for more details.
