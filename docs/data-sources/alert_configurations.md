...
* [atlas-alert-configurations](https://github.com/mongodb/terraform-provider-mongodbatlas/tree/v2.9.0/examples/atlas-alert-configurations)
...
### Threshold Config
* `metric_name` - Human-readable label identifying the metric being checked. Present for stream processor alert types (e.g. `OUTSIDE_STREAM_PROCESSOR_METRIC_THRESHOLD`).
* `mode` - How MongoDB Cloud computes the metric value (e.g. `AVERAGE`).
* `operator` - The operator to apply when checking the current metric value against the threshold value.
  Accepted values are:
    - `GREATER_THAN`
    - `LESS_THAN`
