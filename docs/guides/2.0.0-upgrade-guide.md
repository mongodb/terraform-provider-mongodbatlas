...
- **New `delete_on_create_timeout` attribute**: Multiple resources now support a `delete_on_create_timeout` boolean attribute that controls automatic cleanup behavior when resource creation times out. When set to `true` (default), the provider will automatically delete the partially created resource if creation times out, helping to avoid orphaned resources. When set to `false`, the resource will remain in its partially created state for manual cleanup. This attribute is available in the following resources:
