...
### Current limitation for multi-shard clusters

`use_effective_fields` is not currently supported for multi-shard clusters (more than one `replication_specs` entry). Atlas-managed size values might not stay associated with the intended shard during topology changes. MongoDB is working to remove this limitation.
...
For multi-shard clusters, do not rely on `use_effective_fields` to preserve Atlas-managed `instance_size`, `disk_size_gb`, or `disk_iops` values during topology changes. For shard topology changes on multi-shard clusters, this Terraform documentation currently recommends only tail append, as long as existing shards are not reordered and the operation does not rely on `use_effective_fields` or list-index `lifecycle.ignore_changes` to preserve Atlas-managed size values. Shard removal, including tail delete, is not currently supported as a general Terraform workflow for multi-shard clusters. Use extra care with changes that remove, reorder, move, or replace shards, add a shard before existing shards, or change zone layout, especially when a previous topology change is still in progress. Do not assume that deleting the last Terraform `replication_specs` entry removes the Atlas tail shard, because Atlas shard order can differ from Terraform list order. For significant production topology changes, contact [MongoDB Support](https://www.mongodb.com/docs/atlas/support/#request-support) to review the safest path before applying the change. See [Multi-shard clusters and topology changes](../resources/advanced_cluster.md#multi-shard-clusters-and-topology-changes).
...
~> **IMPORTANT:** Lifecycle ignores address replication specs and region configs by list index. They can prevent Terraform from reverting an Atlas-managed value while topology and ordering remain unchanged, but they are not recommended when adding, removing, reordering, moving, or replacing shards. For significant production topology changes, contact [MongoDB Support](https://www.mongodb.com/docs/atlas/support/#request-support) to review the safest path before applying the change. See [Multi-shard clusters and topology changes](../resources/advanced_cluster.md#multi-shard-clusters-and-topology-changes).
