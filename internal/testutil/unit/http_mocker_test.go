...
func TestRequestInfo_Match(t *testing.T) {
	...
	assert.True(t, req.Match(t, "GET", "2022-06-01", asURL(t, "/v1/cluster/123"), usedVars)) // Exact match
	...
	assert.False(t, req.Match(t, "GET", "2022-06-01", asURL(t, "/v1/cluster/123"), usedVars2)) // API Spec match
...
func TestRequestInfo_MatchQuery(t *testing.T) {
	...
	usedVars := map[string]string{"groupId3": "6746cee66f62fc3c122a3b82"}
	assert.True(t, reqAzure.Match(t, "GET", "2022-06-01", reqURLAzure, usedVars))

	assert.Equal(t, map[string]string{"groupId3": "6746cee66f62fc3c122a3b82"}, usedVars)
	...
	assert.False(t, reqAzure.Match(t, "GET", "2022-06-01", reqURLAws, usedVars))
	assert.Equal(t, map[string]string{"groupId3": "6746cee66f62fc3c122a3b82"}, usedVars)
	...
	assert.True(t, reqAws.Match(t, "GET", "2022-06-01", reqURLAws, usedVars))
}
