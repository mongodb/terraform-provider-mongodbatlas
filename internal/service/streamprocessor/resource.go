...
func (r *streamProcessorRS) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	newStreamProcessorModel, diags := NewStreamProcessorWithStats(ctx, projectID, instanceName, workspaceName, streamProcessorResp, &plan.Timeouts, &plan.DeleteOnCreateTimeout, &plan.FailoverEnabled, &plan.ResumeFromCheckpoint)
	if diags.HasError() {
		resp.Diagnostics.Append(diags...)
		return
	}
...
func (r *streamProcessorRS) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	newStreamProcessorModel, diags := NewStreamProcessorWithStats(ctx, projectID, instanceName, workspaceName, streamProcessor, &state.Timeouts, &state.DeleteOnCreateTimeout, &state.FailoverEnabled, &state.ResumeFromCheckpoint)
	if diags.HasError() {
		resp.Diagnostics.Append(diags...)
		return
	}
