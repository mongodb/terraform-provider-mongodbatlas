...
func TestAccStreamRSStreamConnection_conflictingFields(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      configureKafkaWithBothFields(projectID, instanceName, connectionName, "user", "password", "localhost:9092"),
...
func configureKafkaWithBothFields(projectID, instanceName, connectionName, username, password, bootstrapServers string) string {
	return fmt.Sprintf(`
		resource "mongodbatlas_stream_connection" "test" {
		    project_id = %[1]q
			instance_name = %[2]q
