...
func TestAccStreamDSStreamConnections_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: streamConnectionsDataSourceConfig(configureKafka(projectID, instanceName, "conn-basic", "user", "rawpassword", "localhost:9092,localhost:9092", "earliest", kafkaNetworkingPublic, false)),
...
func TestAccStreamDSStreamConnections_withPageConfig(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: streamConnectionsWithPageAttrDataSourceConfig(configureKafka(projectID, instanceName, "conn-with-page", "user", "rawpassword", "localhost:9092,localhost:9092", "earliest", kafkaNetworkingPublic, false)),
