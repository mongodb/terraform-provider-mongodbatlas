...
func TestPopulateThresholdFromPlan(t *testing.T) {
	apiReqWithBoth := func() *admin.GroupAlertsConfig {
		return &admin.GroupAlertsConfig{
			MetricThreshold: &admin.StreamProcessorMetricThreshold{MetricName: new("STREAM_PROCESSOR_CHANGE_STREAM_LAG")},
			Threshold:       &admin.StreamProcessorMetricThreshold{Threshold: new(1.0)},
		}
	}

	testCases := map[string]struct {
		apiReq                  *admin.GroupAlertsConfig
		expectedMetricThreshold *admin.StreamProcessorMetricThreshold
		...
		"Only threshold in plan": {
			...
			expectedThreshold:       &admin.StreamProcessorMetricThreshold{Threshold: new(1.0), Operator: new("LESS_THAN")},
		},
		"Both in plan, metric wins": {
			apiReq: apiReqWithBoth(),
			metricThreshold: []alertconfiguration.TfMetricThresholdConfigModel{
				{MetricName: types.StringValue("STREAM_PROCESSOR_CHANGE_STREAM_LAG")},
			},
