	...
	testCases := map[string]struct {
		...
		"GCP Confluent with all required fields": {
			tfModel: &streamprivatelinkendpoint.TFModel{
				...
				ServiceAttachmentUris: types.ListValueMust(types.StringType, []attr.Value{
					types.StringValue("projects/test-project/regions/us-west1/serviceAttachments/test-attachment"),
				}),
		...
		"GCP Confluent missing service_attachment_uris": {
			...
			expectError: true,
			errorCount:  2, // Error for neither field provided and error for service_attachment_uris required for GCP
		},
		...
		"AWS Confluent": {
			tfModel: &streamprivatelinkendpoint.TFModel{
				Provider:              types.StringValue(constant.AWS),
				Vendor:                types.StringValue(streamprivatelinkendpoint.VendorConfluent),
				ServiceAttachmentUris: types.ListNull(types.StringType), // Should not trigger GCP validation
		...
		"AWS Confluent with service_attachment_uris": {
			tfModel: &streamprivatelinkendpoint.TFModel{
				Provider: types.StringValue(constant.AWS),
				Vendor:   types.StringValue(streamprivatelinkendpoint.VendorConfluent),
				ServiceAttachmentUris: types.ListValueMust(types.StringType, []attr.Value{
				...
				ServiceEndpointId: types.StringNull(),
				DnsDomain:         types.StringValue("example.com"),
				Region:            types.StringValue("us-west2"),
			},
			expectError: true,
			errorCount:  1,
		},
