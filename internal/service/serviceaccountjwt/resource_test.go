...
func protoV6FactoriesWithEcho() map[string]func() (tfprotov6.ProviderServer, error) {
	...
	maps.Copy(factories, acc.TestAccProviderV6Factories)
	factories["echo"] = echoprovider.NewProviderServer()
	return factories
}

func preCheckSACreds(t *testing.T) {
	t.Helper()
	acc.PreCheckBasic(t)
	if !acc.HasSACreds() {
		t.Skip("MONGODB_ATLAS_CLIENT_ID and MONGODB_ATLAS_CLIENT_SECRET required")
	}
}
...
func TestAccServiceAccountJWT_providerCredentials(t *testing.T) {
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:                 func() { preCheckSACreds(t) },
		TerraformVersionChecks:   versionChecks,
		ProtoV6ProviderFactories: protoV6FactoriesWithEcho(),
