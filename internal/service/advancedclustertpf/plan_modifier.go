...
func useStateForUnknowns(ctx context.Context, diags *diag.Diagnostics, state, plan *TFModel) {
	...
	// `replication_specs` is handled by index to allow using partial state for "changed" specs
	if slices.Contains(keepUnknown, "replication_specs") {
		// Always avoid copying `id` fields even when there are no changes to the spec
		var keepUnknownReplicationSpecs = []string{"zone_id", "id", "external_id"}
