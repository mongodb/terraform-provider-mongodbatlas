...
type TypeAtTerraformPath interface {
	TypeAtTerraformPath(context.Context, *tftypes.AttributePath) (attr.Type, error)
}

func AttributePath(ctx context.Context, tfType *tftypes.AttributePath, schema TypeAtTerraformPath) (path.Path, diag.Diagnostics) {
	fwPath := path.Empty()
