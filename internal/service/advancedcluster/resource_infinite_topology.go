...
// verifyInfiniteTopologyOnUpdate covers the update whose edition only Atlas knows, e.g. after an import,
// where neither the plan nor the state can reveal that the cluster runs the INFINITE edition.
func (r *rs) verifyInfiniteTopologyOnUpdate(ctx context.Context, diags *diag.Diagnostics, state, plan *TFModel) {
	clusterType := plan.ClusterType.ValueString()
	if !isShardedClusterType(clusterType) {
		return
	}
