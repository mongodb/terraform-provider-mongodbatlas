...
func Resource() resource.Resource {
	return &rs{
		RSCommon: config.RSCommon{
			ResourceName: "stream_processor_api",
		},
	}
}
...
func (r *rs) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	callParams := config.APICallParams{
		...
		RelativePath:  "/api/atlas/v2/groups/{groupId}/streams/{tenantName}/processor",
...
func readAPICallParams(model any) *config.APICallParams {
	...
	pathParams := map[string]string{
		...
		"tenantName": m.TenantName.ValueString(),
...
func formatIDAttributes(model any) string {
	m := model.(*TFModel)
	return fmt.Sprintf("group_id=%q, tenant_name=%q, name=%q",
		m.GroupId.ValueString(),
		m.TenantName.ValueString(),
		m.Name.ValueString(),
	)
}
