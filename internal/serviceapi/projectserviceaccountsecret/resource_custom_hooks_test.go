...
func TestPostReadAPICall(t *testing.T) {
	t.Run("resource", func(t *testing.T) {
		hook := postReadHook(t, projectserviceaccountsecret.Resource())
		assertSecretLookup(t, func(secretID string, result autogen.APICallResult) autogen.APICallResult {
			state := &projectserviceaccountsecret.TFModel{SecretId: types.StringValue(secretID)}
			return hook.PostReadAPICall(autogen.HandleReadReq{State: state}, result)
		})
	})

	t.Run("data source", func(t *testing.T) {
		hook := postReadHook(t, projectserviceaccountsecret.DataSource())
		assertSecretLookup(t, func(secretID string, result autogen.APICallResult) autogen.APICallResult {
			state := &projectserviceaccountsecret.TFDSModel{SecretId: types.StringValue(secretID)}
			return hook.PostReadAPICall(autogen.HandleReadReq{State: state}, result)
		})
	})
}
