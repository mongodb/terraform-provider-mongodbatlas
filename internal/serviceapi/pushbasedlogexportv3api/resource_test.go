...
func checkDestroy(state *terraform.State) error {
	...
	for _, rs := range state.RootModule().Resources {
		if rs.Type == "mongodbatlas_push_based_log_export_api" {
			_, _, err := acc.ConnPreview().PushBasedLogExportApi.GetGroupLogIntegration(context.Background(), rs.Primary.Attributes["project_id"], rs.Primary.Attributes["id"]).Execute() // TODO: change to ConnV2() once API is in production
			if err != nil {
