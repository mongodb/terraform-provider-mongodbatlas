...
const resourceName = "mongodbatlas_cluster_old_api.test"

func TestAccClusterOldAPI_basic(t *testing.T) {
	var (
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: configBasic(groupID, clusterName, "M0"),
				Check:  checkBasic(groupID, clusterName, "M0"),
			},
			{
...
func configBasic(groupID, clusterName, instanceSize string) string {
	return fmt.Sprintf(`
		resource "mongodbatlas_cluster_old_api" "test" {
			group_id     = %[1]q
			name         = %[2]q
...
func checkDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "mongodbatlas_cluster_old_api" {
			continue
		}
		groupID := rs.Primary.Attributes["group_id"]
		clusterName := rs.Primary.Attributes["name"]
