...
- **Computability and requiredness overrides**: Override whether an attribute is **required**, **optional**, or **computed** to better match real API behavior. This includes support for attributes that are both **optional + computed**, which is useful for fields that may be omitted by users but are defaulted or populated by the API (for example, defaulted fields on clusters).
