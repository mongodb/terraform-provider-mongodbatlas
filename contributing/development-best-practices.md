...
`PreCreateAPICallHook` and `PreUpdateAPICallHook` receive the request context, so they can make additional API calls, and returning an error from them aborts the operation without sending the request.
