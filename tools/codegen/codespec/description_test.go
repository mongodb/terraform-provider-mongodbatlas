...
func TestEnhanceDescriptions_Resource(t *testing.T) {
	...
	assert.Equal(t, "Required when discriminator_prop is: ALPHA, BETA. Required in all types description.", *attrs[2].Description)
	assert.Equal(t, "Required when discriminator_prop is: BETA. Applicable when discriminator_prop is: ALPHA. Mixed required and optional description.", *attrs[3].Description)
	assert.Equal(t, "Required when discriminator_prop is: GAMMA. Required single type description.", *attrs[4].Description)
...
func TestEnhanceDescriptions_DataSource(t *testing.T) {
	...
	assert.Equal(t, "Applicable when type is: GCS_LOG_EXPORT, S3_LOG_EXPORT. Name of the bucket.", *attrs[1].Description, "data sources always use 'Applicable when'")
}
...
func TestEnhanceDescriptions_NestedDiscriminator(t *testing.T) {
	...
	assert.Equal(t, "Required when auth_type is: OAUTH. OAuth client ID.", *nestedAttrs[1].Description)
}
