...
Orchestrates incorporating an API spec change into an existing autogen resource: the regenerated code plus the complementary artifacts the regeneration does not produce. This skill owns sequencing and commands only — schema and config judgment lives in `autogen-config`, test conventions in `acceptance-test-patterns`, docs and changelog conventions in `pr-and-documentation-standards`.
...
4. **Complementary artifacts**, drafted from the regeneration diff (honoring the artifact selection):
   ...
   - **Examples** — wire new optional attributes through variables (e.g. `default = null`) rather than hardcoding values.
