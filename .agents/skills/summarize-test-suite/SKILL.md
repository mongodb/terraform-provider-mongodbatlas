...
- **Do not classify a *transient* `INVALID_ATTRIBUTE` as category 1.** A minority of `INVALID_ATTRIBUTE` errors are not "the value is wrong" but "the backend is temporarily unable to validate the (valid) value right now, retry later". Judge by **meaning, not exact wording**: does the `Detail` name a concrete, permanent problem with the value (unsupported value, out-of-range, incompatible combination, bad format) → **category 1**; or does it describe a temporary backend condition and invite a retry (data not yet propagated, transient validation failure, "try again later") → **category 4** eventual-consistency flake, since the same config passes on re-run once the backend catches up and no code / test-config change fixes it. Retry-intent phrasing is the signal, in any wording — e.g. `stale`, `please wait`, `try again`, `retry`, `temporarily`, `not yet available`, `in a few minutes`. One canonical example: `Cannot validate cluster compatibility due to stale monitoring data. Please wait a few minutes and try again.` The list is illustrative, not exhaustive — match new phrasings by the same "value is fine, backend can't process it yet, retry" intent. When genuinely ambiguous, keep category 1 and lower confidence (note the specific detail string in the *Why* line).
...
- `INVALID_ATTRIBUTE` is category 1 by default, but a transient variant is category 4 flake — the value is valid, the backend just couldn't validate it yet. Judge by meaning, not exact string: a detail with retry-intent ("value is fine, retry later"; e.g. `stale monitoring data … please wait a few minutes and try again`, or any similar "try again"/"temporarily"/"not yet available" wording) is flake; a concrete-reason detail (unsupported/out-of-range/bad combination) stays red. Match new phrasings by intent, not a fixed keyword list.
