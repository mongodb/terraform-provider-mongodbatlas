...
- **Do not classify `INVALID_ATTRIBUTE` as category 3a.** When a failing test's error context shows `INVALID_ATTRIBUTE`, the provider sent an attribute value the Atlas API rejects — that is a provider or test-config issue, category 1. Tests that use `ExpectError: regexp.MustCompile("INVALID_ATTRIBUTE")` pass cleanly; Step 2's `--- FAIL` filter already excludes them from classification.
...
- `INVALID_ATTRIBUTE` in a failing test's error context is category 1 (code regression) — the provider is sending an attribute the API rejects. Tests that deliberately expect this error via `ExpectError` pass overall and are not classified by Step 2's `--- FAIL` filter.
