...
- `INVALID_ATTRIBUTE` in a failing test's error context is category 1 (code regression) — root cause may be provider code, test config, or a new Atlas API validation, but all cases require investigation. Tests that deliberately expect this error via `ExpectError` pass overall and are not classified by Step 2's `--- FAIL` filter.
