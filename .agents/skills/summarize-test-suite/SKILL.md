...
- **Do not treat an attribute-value mismatch or a plan diff as either red or yellow purely on symptom.** They are also how the enumerated sub-bucket 3b backend states surface (LDAP verify `FAILED`, sample-dataset load `FAILED`, the `stale monitoring data` `INVALID_ATTRIBUTE`). Run the symptom-vs-cause rule: a match to a known-benign 3b signature stays yellow, anything novel escalates to category 1.
