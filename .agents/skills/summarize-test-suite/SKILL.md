...
- **Do not classify a *transient* `INVALID_ATTRIBUTE` as category 1.** A minority of `INVALID_ATTRIBUTE` errors are not "the value is wrong" but "the backend is temporarily unable to validate the (valid) value right now, retry later". Their `Detail` names a temporary backend condition and asks the caller to wait / retry — the canonical shape is `Cannot validate cluster compatibility due to stale monitoring data. Please wait a few minutes and try again.` These are eventual-consistency flake (**category 4**), not a rejected attribute: the same config would pass on a re-run once monitoring data catches up, no code or test-config change fixes it. Trigger only on retry-wording in the detail (`stale`, `Please wait`, `try again`, `temporarily`, `not yet available`) — a plain `INVALID_ATTRIBUTE` with a concrete reason (unsupported value, out-of-range, bad combination) stays category 1. When in doubt, keep category 1 and lower confidence.
...
- `INVALID_ATTRIBUTE` is category 1 by default, but a transient variant whose detail asks to retry (`Cannot validate cluster compatibility due to stale monitoring data. Please wait a few minutes and try again.`) is category 4 flake — the value is valid, the backend just couldn't validate it yet. Gate strictly on retry-wording (`stale` / `Please wait` / `try again`); a concrete-reason `INVALID_ATTRIBUTE` stays red.
