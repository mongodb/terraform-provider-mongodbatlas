...
- Atlas-internal cross-resource propagation lag (e.g. a `stream_processor` step referencing a `stream_connection` created earlier and getting `HTTP 400 STREAM_PROCESSOR_GENERIC_ERROR` _"connection X does not exist"_) is also category 4. The wire shape can be a non-404 HTTP code and a generic Atlas error code; the deciding signal is the `does not exist` / `not found` wording about a resource the same test successfully created in an earlier step of the current execution. Without that prior-create evidence in the log, it stays category 3.
