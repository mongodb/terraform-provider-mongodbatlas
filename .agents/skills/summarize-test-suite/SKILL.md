...
| 5 | **Cleanup** | `still exists` after destroy, `CANNOT_CLOSE_GROUP_ACTIVE_ATLAS_CLUSTERS`, **`MAX_GROUPS_PER_ORG_EXCEEDED`** on `POST /groups` (the org hit its project cap because prior / leaked projects were not reclaimed), plus everything in cleanup jobs from Step 1 |
...
- **Do not classify `MAX_GROUPS_PER_ORG_EXCEEDED` as category 1, 3, or 3a.** On `POST /groups` it means the org is at its project cap — the create is rejected because the org is full, typically because leftover / leaked projects (from this run or previous runs) were not reclaimed. That is an environment-saturation / cleanup condition, **category 5 (cleanup)** — never a provider regression (category 1) and never an API-contract mismatch (category 3a): the request itself is valid, the org just has no room. It is often downstream of a `500` / `503` wave on `POST /groups` that left projects behind; when both appear, report them separately — the 500 / 503 creates as **category 3 (API errors)** and the `MAX_GROUPS_PER_ORG_EXCEEDED` as **category 5 (cleanup)** — this is the canonical cloud-dev-degradation signature and is high-confidence infrastructure noise, not an ambiguity.
...
[only when the org-saturation coverage caveat applies (see rule below):]
*Coverage caveat*: ~<N> project-creating tests never ran (org saturated / `POST /groups` failing), so this run did not validate provider behaviour — a regression could be masked. Re-run once cloud-dev recovers.
...
**Coverage-caveat rule**: emit the `*Coverage caveat*` line (yellow template only) when category 5 `MAX_GROUPS_PER_ORG_EXCEEDED` and/or category 3 `POST /groups` 500 / 503 failures together account for a large share of the failed tests (rule of thumb: ≥50% of failing tests, or the failure clearly blocked most jobs from provisioning projects). It does not change the emoji (still `:yellow_circle:` — no code regression was seen) or the confidence level; it is an independent honesty flag telling on-call the run did not exercise the provider, so a passing-looking result is "not validated," not "all clear." Estimate `<N>` from the count of tests hitting these two indicators.
...
Keep the *Coverage caveat* line (when it applies) until last — it is an on-call honesty flag, so drop it only if the message is still over the cap after steps 1–4.
