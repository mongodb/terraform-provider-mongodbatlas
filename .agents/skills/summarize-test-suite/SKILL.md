...
| 3b | **Backend job / verify failures** | Atlas-side state surfaced through a pass-through provider (the response's own status or error detail, not a provider transform): `status` expected `SUCCESS` got `FAILED` / `validations.N.status` expected `OK` got `FAIL` in the LDAP CA-cert verify tests (`ldapverify`, `ldapconfiguration`); `sample dataset load <id> failed for cluster ...` (an `acc` fixture polls an Atlas load job that reaches `FAILED`); `Cannot validate cluster compatibility due to stale monitoring data`; `OPERATION_INVALID_SHARDS_NO_PRIMARY`. These are yellow only because they are documented, recurring, benign operational states with known owners to escalate to. Relaying the backend's own wording does not by itself make a failure 3b: a novel backend error pattern is an API-side code regression until shown benign (see *Symptom vs cause* below). In the output, count 3b under the *API errors* line and list the test names when ≤3 (a triage glance; recurring signatures map to known backend owners) |
...
**Discipline rule** (symptom vs cause): an assertion failure on an attribute value, an unexpected plan diff, or an `INVALID_ATTRIBUTE` rejection is a symptom, not a cause. It is category 1 (a code regression in the provider *or* in the Atlas API) unless it matches one of the enumerated benign signatures in sub-bucket 3b: LDAP CA-cert verify reporting `FAILED`, the `acc` sample-dataset load job reaching `FAILED`, and stale cluster/monitoring messages (`Cannot validate cluster compatibility due to stale monitoring data`, `OPERATION_INVALID_SHARDS_NO_PRIMARY`). Do not default to 3b merely because the error text is the backend's own wording: a novel backend error pattern (a new error code, a wide break across many tests or packages, or a previously passing suite now failing) is a red flag: an API-side or provider regression until shown otherwise, and it stays category 1. For a 3b attribution to hold, the same test must have passed on recent nights under different backend state, and no relevant code change should have landed in the interim.
...
- **Do not treat an attribute-value mismatch or a plan diff as either red or yellow purely on symptom.** They are also how backend `FAILED` states surface (`status` expected `SUCCESS` got `FAILED` in LDAP verify, unseeded-cluster checks, `unexpected state 'FAILED'`, the `stale monitoring data` `INVALID_ATTRIBUTE`). Run the symptom-vs-cause rule: a match to a known-benign 3b signature stays yellow, anything novel escalates to category 1.
...
• API errors: <N> tests (includes backend job/verify: LDAP CA-cert, sample dataset load, stale cluster monitoring)
...
• API errors: <N> tests (includes backend job/verify like LDAP CA-cert, sample dataset load, stale cluster monitoring; list test names if ≤3, since recurring signatures map to known backend owners)
...
- An attribute-value mismatch or an unexpected plan diff is a symptom, not a root cause: backend failures surface through the same assertion and plan-check output (LDAP verify `FAILED`, sample-data load `FAILED`, stale-monitoring rejections, generic API errors). But only the enumerated sub-bucket 3b signatures are known-benign; any novel backend error pattern defaults to category 1 (an API-side or provider regression) until the same test has passed on recent nights.
- Recognise the recurring backend signatures (sub-bucket 3b) by name so you classify them in one pass: LDAP CA-cert verify (`status` expected `SUCCESS` got `FAILED`, `validations.N.status` expected `OK` got `FAIL`), sample dataset load (`sample dataset load <id> failed for cluster ...`), and stale cluster / monitoring (`Cannot validate cluster compatibility due to stale monitoring data`, `OPERATION_INVALID_SHARDS_NO_PRIMARY`). These have known backend owners to escalate to and never imply a provider regression.
