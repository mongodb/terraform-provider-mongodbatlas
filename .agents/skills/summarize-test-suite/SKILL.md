...
- **Do not classify an Atlas-side rejection of the timeout-triggered delete as category 3 or 5.** When a delete-on-timeout test (name matches `*DeleteOnCreateTimeout*`, `*createTimeoutWithDelete*`, `*deleteOnCreate*`, `*CleanupOnTimeout*`) fails because Atlas rejects the delete with an error like `termination protection is enabled` or `HTTP 400 ATLAS_GENERAL_ERROR` referencing termination protection, that is **category 1** — the provider's timeout-triggered delete path did not handle the constraint correctly (e.g. did not disable termination protection before deleting). This is high-confidence: the test was purpose-built to catch exactly this failure in the cleanup path, and the Atlas rejection proves the delete was attempted and blocked by a provider-side omission. Do not downgrade to category 3 (API contract) or category 5 (cleanup leftover) — the root cause is in the provider code or test config, not in Atlas behaviour.
