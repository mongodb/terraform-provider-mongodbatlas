...
## Autogenerated Resource Configuration

- Let code generation infer import identifier attributes from the read path after aliases are applied. Set `id_attributes` only when the import contract intentionally differs from those path parameters or their order.
- Prefer OpenAPI descriptions. Override a description only when Terraform representation or provider-specific usage differs, such as exposing an API object as a normalized JSON string. Preserve relevant API semantics in the rewritten description.
- Explain each non-obvious `tools/codegen/config.yml` override with an adjacent comment naming the generator, specification, API, or Terraform behavior that requires it.
