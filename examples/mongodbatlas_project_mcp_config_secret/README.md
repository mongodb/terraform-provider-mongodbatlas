# MongoDB Atlas Provider -- Project MCP Config Secret

This example shows how to create an ingress secret for a project-level MCP configuration.

## Important Notes

Unlike `mongodbatlas_project_service_account_secret`, no auto-generated secret needs to be imported: the MCP config's create flow deletes its auto-generated ingress secret server-side, so you always create your first usable secret explicitly with this resource.
