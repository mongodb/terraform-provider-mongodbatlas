...
func TestAccDataSourceMongoDBAtlasCloudProviderSnapshots_withPagination(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccMongoDBAtlasDataSourceCloudProviderSnapshotsConfigWithPagination(projectID, clusterName, description, retentionInDays, 1, 5),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("mongodbatlas_cloud_provider_snapshot.test", "project_id"),
					resource.TestCheckResourceAttrSet("mongodbatlas_cloud_provider_snapshot.test", "cluster_name"),
					resource.TestCheckResourceAttrSet("mongodbatlas_cloud_provider_snapshot.test", "description"),
...
func testAccMongoDBAtlasDataSourceCloudProviderSnapshotsConfigWithPagination(projectID, clusterName, description, retentionInDays string, pageNum, itemPage int) string {
	return fmt.Sprintf(`
		resource "mongodbatlas_cluster" "my_cluster" {
			project_id   = "%s"
			name         = "%s"
