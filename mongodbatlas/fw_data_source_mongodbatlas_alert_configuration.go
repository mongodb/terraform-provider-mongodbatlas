...
func computeAlertConfigurationOutputV2(alert *admin.GroupAlertsConfig, definedOutputs []tfAlertConfigurationOutputModel, defaultLabel string) []tfAlertConfigurationOutputModel {
	resultOutputs := make([]tfAlertConfigurationOutputModel, len(definedOutputs))
	for i, defined := range definedOutputs {
		resultOutput := tfAlertConfigurationOutputModel{}
		resultOutput.Type = defined.Type
		...
		if outputValue := outputAlertConfigurationV2(alert, resultOutput.Type.ValueString(), resultOutput.Label.ValueString()); outputValue != "" {
			resultOutput.Value = types.StringValue(outputValue)
		}
		resultOutputs[i] = resultOutput
	}
...
func outputAlertConfigurationV2(alert *admin.GroupAlertsConfig, outputType, resourceLabel string) string {
	if outputType == "resource_hcl" {
		return outputAlertConfigurationResourceHclV2(resourceLabel, alert)
	}
	if outputType == "resource_import" {
		return outputAlertConfigurationResourceImportV2(resourceLabel, alert)
	}
...
func outputAlertConfigurationResourceHclV2(label string, alert *admin.GroupAlertsConfig) string {
	f := hclwrite.NewEmptyFile()
	root := f.Body()
	resource := root.AppendNewBlock("resource", []string{"mongodbatlas_alert_configuration", label}).Body()
	...
	for _, matcher := range alert.Matchers {
		appendBlockWithCtyValues(resource, "matcher", []string{}, convertMatcherToCtyValuesV2(matcher))
	}

	if alert.MetricThreshold != nil {
		appendBlockWithCtyValues(resource, "metric_threshold_config", []string{}, convertMetricThresholdToCtyValuesV2(*alert.MetricThreshold))
	}

	if alert.Threshold != nil {
		appendBlockWithCtyValues(resource, "threshold_config", []string{}, convertThresholdToCtyValuesV2(alert.Threshold))
	}

	for i := 0; i < len(alert.Notifications); i++ {
		appendBlockWithCtyValues(resource, "notification", []string{}, convertNotificationToCtyValuesV2(&alert.Notifications[i]))
	}
...
func outputAlertConfigurationResourceImportV2(label string, alert *admin.GroupAlertsConfig) string {
	return fmt.Sprintf("terraform import mongodbatlas_alert_configuration.%s %s-%s\n", label, *alert.GroupId, *alert.Id)
}
...
func convertMatcherToCtyValuesV2(matcher map[string]interface{}) map[string]cty.Value {
...
func convertMetricThresholdToCtyValuesV2(metric admin.ServerlessMetricThreshold) map[string]cty.Value {
	return map[string]cty.Value{
		"metric_name": cty.StringVal(*metric.MetricName),
		"operator":    cty.StringVal(*metric.Operator),
		"threshold":   cty.NumberFloatVal(*metric.Threshold),
...
func convertThresholdToCtyValuesV2(threshold *admin.GreaterThanRawThreshold) map[string]cty.Value {
	return map[string]cty.Value{
		"operator":  cty.StringVal(*threshold.Operator),
		"units":     cty.StringVal(*threshold.Units),
