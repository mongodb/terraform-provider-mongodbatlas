...
`mongodbatlas_cloud_backup_schedule` provides a resource that enables you to view and modify the snapshot schedule and retention settings for an Atlas cluster with Cloud Backup enabled.  A default policy is created automatically when Cloud Backup is enabled for the cluster.
...
## Example Usage - Create a Cluster with not policies (it will import the autogenerated policy)
