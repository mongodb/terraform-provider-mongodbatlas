...
* `config_database` - (Optional) Required for `DATABASE` type. The name of the watched MongoDB database.
* `config_collection` - (Optional) Optional for `DATABASE` type. The name of the MongoDB collection that the trigger watches for change events. The collection must be part of the specified database.
