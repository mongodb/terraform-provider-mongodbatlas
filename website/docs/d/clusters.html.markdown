...
~> **IMPORTANT:**
<br> &#8226; Multi Region Cluster: The `mongodbatlas_cluster` resource doesn't expose the `container_id` for each region utilized by the cluster. For access to `container_id`, we recommend to use the [`mongodbatlas_advanced_cluster`](https://registry.terraform.io/providers/mongodb/mongodbatlas/latest/docs/data-sources/advanced_clusters) data source instead.
