...
3. Perform an import from your existing `mongodbatlas_cloud_provider_snapshot_backup_policy` to `mongodbatlas_cloud_backup_schedule`
```bash
    terraform import mongodbatlas_cloud_backup_schedule.test <project_id>-<cluster_name>
```

4. Do a terraform plan (no changes detected)
   **NOTE** If you imported without policies since now it's optional, it's possible that it will detect changes after terraform plan
