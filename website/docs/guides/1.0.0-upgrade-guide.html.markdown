...
1. Since the schema it's different from `mongodbatlas_cloud_provider_snapshot_backup_policy` in the resource `mongodbatlas_cloud_backup_schedule`, here the following examples from the old and the new configuration:
...
The following changes are:
    1. The IDs for policy and policies items are not necessary to set anymore.
    2. We separated the policies items into a single object for policy item `hourly`, `daily`, `weekly` and `monthly`
    3. The `frequency_type` for a policy item is not necessary to be set.

2. Remove your existing `mongodbatlas_cloud_provider_snapshot_backup_policy` resources from terraform state
   **NOTE** before doing any terraform state change please do a backup
...
3. Change the version of provider to the v1.0.0

4. In order to import the new resource, it needs to change the configuration file from the `mongodbatlas_cloud_provider_snapshot_backup_policy` config to `mongodbatlas_cloud_backup_schedule` with the example shared in the first step.

5. Perform an import of your existing `mongodbatlas_cloud_provider_snapshot_backup_policy` to `mongodbatlas_cloud_backup_schedule`
```bash
    terraform import mongodbatlas_cloud_backup_schedule.test <project_id>-<cluster_name>
```

6. Do a terraform plan (no changes detected)
