...
for file in "$TEMPLATE_FOLDER_PATH/ephemeral-resources"/*; do
    filenameTemplate=$(basename -- "$file")
    filename="${filenameTemplate%.*}"
    if [ -f "docs-out/ephemeral-resources/${filename}" ]; then
        printf "Ephemeral resource file moved: %s\n" "${filename}"
        mv "docs-out/ephemeral-resources/${filename}" "docs/ephemeral-resources/${filename}"
    fi
done
...
for file in docs-out/ephemeral-resources/*; do
    if [ -f "$file" ]; then
        filename=$(basename -- "$file")
        if [[ "$filename" == *_api.md || "$filename" == *_api_v*.md ]]; then
            printf "Autogenerated ephemeral resource file moved: %s\n" "${filename}"
            mv "$file" "docs/ephemeral-resources/${filename}"
        fi
    fi
done
